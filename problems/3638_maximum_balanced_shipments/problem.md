...
- Hint 1: Use a monotonic stack to find for each end index i the nearest previous index j with weight[j] > weight[i] (set j = -1 if none).
- Hint 2: Then set dp[i] = best[j] + 1 (using best[-1] = 0), and update best[i] = max(best[i-1], dp[i]); the result is best[n-1].
