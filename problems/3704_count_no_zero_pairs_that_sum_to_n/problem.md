...
- Hint 1: Use digit DP over the decimal representation of n.
- Hint 2: At each digit, track whether a carry is present and whether a or b has used a zero so far.
- Hint 3: Transition by choosing digits for a and b that add up (with carry) to the current digit of n.
- Hint 4: Subtract the cases where either number ends up being zero-containing when n itself is no-zero.
