...
- Hint 1: For each query [x, y], if x > y, swap x and y. Now, we can assume that x <= y.
- Hint 2: For each query [x, y], if x == y or heights[x] < heights[y], then the answer is y since x ≤ y.
- Hint 3: Otherwise, we need to find the smallest index t such that y < t and heights[x] < heights[t]. Note that heights[y] <= heights[x], so heights[x] < heights[t] is a sufficient condition.
- Hint 4: To find index t for each query, sort the queries in descending order of y. Iterate over the queries while maintaining a monotonic stack which we can binary search over to find index t.
