...
- Hint 1: Since swaps inside the same array are free, positions do not matter; only frequencies matter.
- Hint 2: Let cnt1[v] and cnt2[v] be frequencies. If cnt1[v] + cnt2[v] is odd for any v, return -1.
- Hint 3: Let diff[v] = abs(cnt1[v] - cnt2[v]). Each cross swap fixes two mismatches, so minimum cost = sum(diff[v]) / 4.
