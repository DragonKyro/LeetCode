...
- Hint 1: Notice that the Manhattan distance between two points [x_i, y_i] and [x_j, y_j] is  max({x_i - x_j + y_i - y_j, x_i - x_j - y_i + y_j, - x_i + x_j + y_i - y_j, - x_i + x_j - y_i + y_j}).
- Hint 2: If you replace points as [x_i - y_i, x_i + y_i] then the Manhattan distance is max(max(x_i) - min(x_i), max(y_i) - min(y_i)) over all i.
- Hint 3: After those observations, the problem just becomes a simulation. Create multiset of points [x_i - y_i, x_i + y_i], you can iterate on a point you might remove and get the maximum Manhattan distance over all other points.
