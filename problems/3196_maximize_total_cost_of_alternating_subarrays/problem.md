...
- Hint 3: Let dp[i][0/1] be the largest sum we can get for prefix nums[0..i], where dp[i][0] is the maximum if the i^th element wasn't flipped, and dp[i][1] is the maximum if the i^th element was flipped.
- Hint 4: Based on the restriction:
- dp[i][0] = max(dp[i - 1][0], dp[i - 1][1]) + nums[i]
- dp[i][1] = dp[i - 1][0] - nums[i]
- Hint 5: The initial state is:
