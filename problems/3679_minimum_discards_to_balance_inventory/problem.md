...
- Hint 1: Use a sliding window of up to w days with two pointers left and right to represent the current interval.
- Hint 2: Maintain a hash map cnt from item type to its current count in the window. When you advance right to day i, do cnt[arrivals[i]]++.
- Hint 3: If the window size exceeds w (i.e. right - left + 1 > w), shrink it by doing cnt[arrivals[left]]-- and then left++.
- Hint 4: After each increment, check if cnt[arrivals[right]] > m. If so, we must discard the current arrival.
