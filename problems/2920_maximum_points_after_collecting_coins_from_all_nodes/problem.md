...
- Hint 2: Note that the value of each node = 14 dp[x][t] is always 0.
- Hint 3: General equation will be: dp[x][t] = max((coins[x] >> t) - k + sigma(dp[y][t]), (coins[x] >> (t + 1)) + sigma(dp[y][t + 1])) where nodes denoted by y in the sigma, are the direct children of node x.
