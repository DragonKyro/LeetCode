...
- Hint 1: Find a way to calculate f(n, i) which is the total number of numbers in [1, n] when the i^th bit is set in O(log(n)) time.
- Hint 2: Use binary search to find the last number for each query (and there might be one “incomplete” number for the query).
- Hint 3: Use a similar way to find the product (we only need to save the sum of exponents of power of 2).
