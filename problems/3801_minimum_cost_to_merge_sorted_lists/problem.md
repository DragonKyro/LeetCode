...
- Hint 3: Let dp[mask] represent the minimum cost to merge all lists in mask
- Hint 4: dp[mask] = min(dp[s] + dp[mask ^ s] + cost(s, mask ^ s)), where s and mask ^ s are nonempty disjoint submasks whose union is mask
