...
- Hint 1: Maintain a map from itemId to its active bids.
- Hint 2: For each item, use a data structure ordered by (bidAmount, userId) to get the highest bidder efficiently.
- Hint 3: On addBid or updateBid, remove the old entry (if any) and insert the new one.
- Hint 4: On removeBid, delete the corresponding entry; return the top element for getHighestBidder.
