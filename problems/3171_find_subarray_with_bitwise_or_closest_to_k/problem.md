...
- Hint 1: Let dp[i] be the set of all the bitwise OR of all the subarrays ending at index i.
- Hint 2: We start from nums[i], taking the bitwise OR result by including elements one by one from i towards left. Notice that only unset bits can become set on adding an element, and set bits never become unset again.
- Hint 3: Hence dp[i] can contain at most 30 elements.
