...
- Hint 1: Let dp[k][i] be the minimum score to partition the first i elements into k subarrays. The transition is dp[k][i] = min(dp[k-1][j] + value(subarray nums[j...i-1])) for j < i.
- Hint 2: The naive DP approach takes O(K*N^2), which is too slow. Look for optimizations applicable to partitioning problems.
