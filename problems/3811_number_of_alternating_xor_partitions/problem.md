...
- Hint 2: Compute prefix XORs so the XOR of a block [l..r] can be obtained as pref[r] ^ pref[l - 1].
- Hint 3: For each position, try ending a block whose XOR equals the expected target, and transition to the next state (alternating between target1 and target2).
