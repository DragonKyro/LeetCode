...
- Hint 1: Define left[i][d] as the maximum length of an alternating subarray ending at index i, where d = 0 means the last comparison is . Define right[i][d] similarly for subarrays starting at i.
- Hint 2: Fill left from left to right and right from right to left; if adjacent values are equal, the alternating chain must restart since == is invalid.
- Hint 3: Try removing each index r: if nums[r - 1] , giving length left[r - 1][0] + right[r + 1][1]; if nums[r - 1] > nums[r + 1], connect with pattern > <, giving left[r - 1][1] + right[r + 1][0].
- Hint 4: Also consider not removing any element by taking the maximum value over all left[i][d].
