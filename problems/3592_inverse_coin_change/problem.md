...
- Hint 1: Observe that for the smallest denomination c, you must have numWays[c] == 1.
- Hint 2: Find the smallest c > 0 with numWays[c] == 1 and append c to your ans list.
- Hint 3: "Remove" that coin’s contribution by doing, for each s from c up to n: numWays[s] -= numWays[s - c]
- Hint 4: Repeat: pick the next smallest c with numWays[c] == 1, remove it, and so on.
- Hint 5: At the end, if numWays is all zeros, your ans is valid; otherwise, return an empty array.
