...
- Hint 2: Let dp[i] be the maximum money robbable considering houses up to index i
- Hint 3: Initialize dp[i] with nums[i], then compare with skipping the house: dp[i - 1]
- Hint 4: If colors[i] != colors[i - 1], allow taking adjacent house: nums[i] + dp[i - 1]
- Hint 5: Always allow non-adjacent take: nums[i] + dp[i - 2]
- Hint 6: Answer is dp[n - 1]
