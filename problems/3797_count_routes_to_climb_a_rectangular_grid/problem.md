...
- Hint 2: Let dp[r][c][0] be the number of ways to reach (r, c) where the last move came from row r + 1 (moved up), and let dp[r][c][1] be the number of ways where the last move stayed on row r.
- Hint 3: Make computations faster using prefix sums over columns to aggregate contributions from cells within Euclidean distance d.
- Hint 4: Combine dp[r][c][0] and dp[r][c][1] for all columns c in row r to produce the dp values used for row r - 1.
