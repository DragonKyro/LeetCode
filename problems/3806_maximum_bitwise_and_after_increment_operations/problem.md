...
- Hint 2: Iterate bits from highest to lowest and try setting the current bit in a candidate res.
- Hint 3: To test a candidate, for each num compute the minimal increments needed so that (num | candidate) == candidate; take the smallest m costs and check if their sum <= k.
- Hint 4: If feasible, keep the bit in res and continue with accumulated bits.
