...
- Hint 2: Define a 2D array freq[node][weight] which saves the frequency of each edge weight on the path from the root to each node.
- Hint 3: The frequency of edge weight w on the path from a to b is equal to freq[a][w] + freq[b][w] - freq[lca(a,b)][w] * 2, where lca(a,b) is the lowest common ancestor of a and b in the tree.
- Hint 4: lca(a,b) can be calculated using binary lifting algorithm or Tarjan algorithm.
