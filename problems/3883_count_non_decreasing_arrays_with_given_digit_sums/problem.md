...
- Hint 2: Group every number from 0 to 5000 by its digit sum into sorted vectors once.
- Hint 3: dp[j] holds the number of ways ending exactly at the j-th candidate for the current position.
- Hint 4: Build prefix sums on the previous dp, then use upper_bound on the sorted previous list to count all values <= current.
