...
- Hint 2: Let dp[x][c] be the maximum sum we can get for the subtree rooted at node x, where c is a boolean representing whether the edge between node x and its parent (if any) is selected or not.
- Hint 3: dp[x][c] = max(sum(dp[y][cy]) + v(nums[x], sum(cy) + c))
- where cy is 0 or 1.
- When sum(cy) + c is odd, v(nums[x], sum(cy) + c) = nums[x] XOR k.
- When sum(cy) + c is even, v(nums[x], sum(cy) + c) = nums[x].
- Hint 4: There’s also an easier solution - does the parity of the number of elements where nums[i] XOR k > nums[i] help?
