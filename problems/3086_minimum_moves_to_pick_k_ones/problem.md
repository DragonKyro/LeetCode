...
- Hint 1: Ones created using a change require 2 moves. Hence except for the immediate neighbors of the index where we move all the ones, we should try to use change operations.
- Hint 2: For some subset of ones, it is always better to move the ones to the median position.
- Hint 3: We only need to be concerned with the indices where nums[i] == 1.
