...
- Hint 1: First, compute all positive divisors of n and sort them into a list divs.
- Hint 2: Use a recursive search dfs(start, picked, prod, path) that picks the next divisor from divs[start...], multiplies it into prod, and appends to path until you have k factors whose product is n.
- Hint 3: During the search, keep track of the best path that minimizes max(path) – min(path) and return it.
