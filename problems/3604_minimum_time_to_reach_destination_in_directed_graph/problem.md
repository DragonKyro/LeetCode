...
- Hint 1: Use the Dijkstra algorithm over states (node, time).
- Hint 2: At node u with current time t, you can only use an edge [u, v, start, end] if t <= end.
- Hint 3: If t < start, wait until start, then traverse (arriving at start + 1).
