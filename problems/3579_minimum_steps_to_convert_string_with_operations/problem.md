...
- Hint 2: Do DP on disjoint substrings of word1. For the DP, we try both the substring and its reversed version (just add one extra operation)
- Hint 3: First we swap pairs like (word1[i], word2[i]) and (word1[j], word2[j]) where word1[i] == word2[j] and word2[i] == word1[j]
