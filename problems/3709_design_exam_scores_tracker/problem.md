...
- Hint 1: Maintain two arrays: times (append each time) and prefix where prefix[i] is sum of scores up to index i.
- Hint 2: Use binary search to find indices: l = lower_bound(times, startTime) and r = upper_bound(times, endTime) - 1.
- Hint 3: If l > r there are no exams, so return 0; otherwise compute total using the prefix.
