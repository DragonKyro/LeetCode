...
Given an array queries, where queries[j] = [p_j, q_j, limit_j], your task is to determine for each queries[j] whether there is a path between p_j and q_j_ such that each edge on the path has a distance strictly less than limit_j .
