...
- Hint 1: Precompute depth[i] for each nums[i] by applying popcount until you reach 1.
- Hint 2: Maintain six Fenwick trees fenw[0] through fenw[5], where fenw[d] stores a 1 at index i iff depth[i] == d.
- Hint 3: For an update [2, idx, val], remove index idx from its old fenw[old_depth] and insert into fenw[new_depth]; for a query [1, l, r, k], return fenw[k].query(r) - fenw[k].query(l-1).
