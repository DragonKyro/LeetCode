...
- Hint 1: Let dp[i][j] denote the number of arrays of length i with j inversions.
- Hint 2: dp[i][j] = dp[i - 1][j] + dp[i - 1][j - 1] + … + dp[i - 1][0].
- Hint 3: dp[i][j] = 0 if for some x, requirements[x][0] == i and requirements[x][1] != j.
