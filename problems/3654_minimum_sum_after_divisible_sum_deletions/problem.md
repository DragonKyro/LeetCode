...
- Hint 1: A subarray sum is divisible by k precisely when the prefix sums at its two endpoints have the same remainder mod k.
- Hint 2: Define dp[i] as the minimum total remaining sum after processing the first i elements.
- Hint 3: Keep a map from each remainder to the best (smallest) value of dp[j] - prefixSum[j] you've seen for that remainder to update dp[i] in O(1).
