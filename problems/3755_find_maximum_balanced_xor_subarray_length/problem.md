...
- Hint 1: In one pass maintain prefix pxor and prefix diff = (#evens − #odds) and record in a hash‑map the earliest index where each state (pxor,diff) occurs.
- Hint 2: At each index, if the current state (pxor,diff) is already in the map, update the max length as the current index minus the recorded index.
