...
- Hint 1: Fix a bit from the range [0, 31], then count the number of elements of nums that have bit set in them.
- Hint 2: bit is set in integer x if and only if 2^bit AND x == 2^bit, where AND is the bitwise AND operation.
