...
- Hint 2: Any subarray with >= 2 distinct values has a unique strict-increase boundary (i, j), so the value sequence is identified by that boundary and how many elements you take from each edge.
- Hint 3: Count single-value sequences per run by testing (L*val) % k == 0.
