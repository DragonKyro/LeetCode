...
- Hint 1: What happens when min(nums) * multiplier > max(nums)?
- Hint 2: A cycle of operations begins.
- Hint 3: Simulate until min(nums) * multiplier > max(nums), then greedily distribute remaining multiplications.
