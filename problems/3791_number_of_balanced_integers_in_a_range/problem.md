...
- Hint 2: Let f(x) be the number of balanced integers in [1, x]; the answer is f(high) - f(low - 1).
- Hint 3: Track the difference sum(odd positions) - sum(even positions) while building from the most significant digit, and require it to be zero at the end.
