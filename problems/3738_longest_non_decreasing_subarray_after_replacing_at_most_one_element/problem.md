...
- Hint 2: Create a prefix array where pref[i] denotes the maximum-length non-decreasing subarray ending at i
- Hint 3: Create a suffix array where suff[i] denotes the maximum-length non-decreasing subarray starting at i
- Hint 4: Let the initial maximum be max(max(pref), max(suff))
- Hint 5: For each index i, try to combine the prefix ending at i - 1 and the suffix starting at i + 1 if possible; also consider the maximum without combining them
