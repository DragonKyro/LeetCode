...
- Hint 1: Think of each number as a mask: treat nums[i] as a bitmask.
- Hint 2: Create an array dp of size 1 << B, where B is your bit‑width.
- Hint 3: Initialize dp[mask] to the maximum nums[i] exactly equal to that mask, or 0 if none.
- Hint 4: For each m, propagate to all its super‑masks M: dp[m] = max(dp[m], dp[M])
- Hint 5: For a number x with mask mx, compute its "complement mask" as cm = ~mx & ((1 << B)-1).
- Hint 6: The best disjoint partner is then dp[cm].
- Hint 7: Loop over all x in nums, look up dp[cm], and track the maximum x * partner.
