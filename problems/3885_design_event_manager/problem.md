...
- Hint 1: Push every new priority version into the heap without removing old ones.
- Hint 2: In pollHighest, keep popping until the top matches the live priority stored in the map.
- Hint 3: Use {priority, -eventId} ordering so the max-heap returns the highest priority first and the smallest eventId on ties.
